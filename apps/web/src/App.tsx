...
/** Real cockpit shell: fetches the CodeHarness API and renders skills + agent dialogue
 *  + platform. The standalone hi-fi view lives at apps/console/index.html. */
export function App() {
  ...
  if (err) return <main style={{ ...wrap, padding: 28 }}><h1>CodeHarness Cockpit</h1><p style={{ color: '#F2A65A' }}>{err}</p></main>;
  if (!d) return <main style={{ ...wrap, padding: 28 }}><p style={{ color: 'rgba(230,237,243,.5)' }}>Loading platform…</p></main>;
  ...
  return (
    <main style={wrap}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr 360px' }}>
        <section style={{ padding: 18, borderRight: '1px solid rgba(230,237,243,.1)' }}>
          ...
          {d.agents.map((a: any) => (
            <div key={a.id} style={{ background: '#18242F', border: '1px solid rgba(230,237,243,.1)', borderRadius: 9, padding: 11, marginBottom: 8 }}>
              ...
              <div style={{ fontSize: 11.5, marginTop: 5 }}><span style={{ color: 'rgba(230,237,243,.34)' }}>can </span>{a.does}</div>
              <div style={{ fontSize: 11.5, marginTop: 3 }}><span style={{ color: 'rgba(230,237,243,.34)' }}>never </span><span style={{ color: '#F2A65A' }}>{a.never}</span></div>
            </div>
          ))}
          {d.skills.map((s: any) => (
          ...
          {d.conv.messages.map((m: any) => (
            <div key={m.seq} style={{ borderLeft: '1px solid rgba(230,237,243,.1)', paddingLeft: 16, paddingBottom: 15, marginLeft: 6 }}>
              <div style={{ ...mono, fontSize: 11.5 }}>
                <span style={{ color: 'rgba(230,237,243,.34)' }}>#{m.seq} </span>
                <b style={{ color: ROLE[m.from] || '#9FB0BF' }}>{m.from}</b>
                {m.to && <span style={{ color: 'rgba(230,237,243,.56)' }}> → {m.to}</span>}
                <span style={{ float: 'right', fontSize: 9.5, textTransform: 'uppercase', color: 'rgba(230,237,243,.34)', border: '1px solid rgba(230,237,243,.1)', borderRadius: 5, padding: '1px 6px' }}>{m.type}</span>
              </div>
